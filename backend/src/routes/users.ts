...
router.post("/", async (req, res) => {
  try {
    ...
    if (JWT_TOP_SECRET_KEY) {
      const verifyToken = jsonwebtoken.sign({ email: userEntry.email }, JWT_TOP_SECRET_KEY);
      const userToAdd = {
      ...
      await sendVerificationMail(userToAdd.firstName, userToAdd.email, verifyToken);
